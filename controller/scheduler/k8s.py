...
class KubeHTTPClient(AbstractSchedulerClient):
    ...
    def _scale_rc(self, rc, namespace):
        ...
        conn_scalepod.request('PUT', '/api/' + self.apiversion + '/namespaces/default/' +
                              'replicationcontrollers/' + name, headers=headers,
                              body=json.dumps(rc))
    ...
    def scale(self, name, image, command, **kwargs):
        ...
        try:
            ...
            logger.log(logging.ERROR, "{}: {}".format(app_name, e))
            raise RuntimeError(err)
    ...
    def _create_rc(self, name, image, command, **kwargs):
        ...
        l["healthurl"] = kwargs.get('healthurl', '/health-check')
        l["healthtimeout"] = kwargs.get('healthtimeout', 1)
        l["healthinitialdelay"] = kwargs.get('healthinitialdelay', 30)
